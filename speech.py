...
def transcribe(audio_path: str | Path) -> TranscriptionResult:
    ...
    try:
        ...
        raise STTError("Sarvam request timed out.") from exc

    except requests.HTTPError as exc:
        message = exc.response.text if exc.response else str(exc)
        raise STTError(f"Sarvam HTTP Error: {message}") from exc

    except requests.RequestException as exc:
        raise STTError(f"Network error: {exc}") from exc

    except ValueError as exc:
        raise STTError("Sarvam returned invalid JSON.") from exc